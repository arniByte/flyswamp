...
Плодовая мушка с настоящей проводкой мозга живёт на ночном болоте. Она ищет по запаху гнилые фрукты и учится не садиться на росянку, которая пахнет почти как еда. Обучение идёт через реальный грибовидный мозг (mushroom body) из коннектома **MaleCNS v1.0**: дофамин меняет синапсы KC→MBON. Все 165 122 нейрона MaleCNS и 15 млн связей между ними считаются спайковой моделью Shiu et al. 2024 в фоновом потоке браузера. В режиме «рентген» светятся сомы 141 000 из них, каждая от своих спайков. Поведение и обучение пока ведёт прототипная rate-модель цепи грибовидного тела на 5 614 нейронах.
...
npm test           # 19 тестов: спайковый движок, граф CNS для браузера, цепь, обучение, контроли, мир
npm run build      # статическая сборка в web/dist
```

Данные прототипа лежат в `web/public/data` (около 14 МБ), бэкенд не нужен: всё считается в браузере. Граф целого CNS (31 МБ) в git не входит, его собирает `scripts/setup.sh webgraph` из данных MaleCNS. Без него игра работает, но без спайкового CNS.
...
| нейроны | в рентгене и мозге легенда справа включает и выключает группы цепи MB, наведение на сому показывает тип, bodyId и активность; сомы остального CNS тоже отвечают на наведение |
...
**Модель и допущения:**
- Поведение и обучение — rate-модель, а не LIF. Вспышки на скелетах 5 614 нейронов цепи семплируются из частот и служат только визуализацией.
- Спайковый CNS работает рядом с ней. Из мира в него идёт только вкус: пока муха ест, правые сахарные GRN получают 150 Hz, как в бенчмарке P2. В HUD виден ответ мотонейрона хоботка MN9, телом CNS пока не управляет. На скорости ×2 и выше мозг может не успевать за миром, HUD это пишет.
